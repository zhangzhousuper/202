                ...
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int l = 0; l <= SHOrder;l++)
                        for (int m = -l; m <= l; m++)
                        {
                            ...
                            SHCoeffiecents[idx] += Le * basisFunc * dw;
                        }
                }
            }
        }
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            //eg m_TransportSHCoeffs.resize(SHCoeffLength, mesh->getVertexCount());
            MatrixXf m_IndirectCoeffs;
            m_IndirectCoeffs.resize(SHCoeffLength, mesh->getVertexCount());
            for (int bo = 1; bo < m_Bounce; bo++) {
                for (int vtx = 0; vtx < mesh->getVertexCount(); vtx++) {
                    const Point3f &v = mesh->getVertexPositions().col(vtx);
                    const Normal3f &n = mesh->getVertexNormals().col(vtx);
                    ...
                    for (int j = 0; j < SHCoeffLength; j++)
                    {
                        m_IndirectCoeffs.col(vtx).coeffRef(j) = coeff[j] - m_IndirectCoeffs.col(vtx).coeffRef(j);
                    }
                }
