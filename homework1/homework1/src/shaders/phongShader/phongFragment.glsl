...
float findBlocker( sampler2D shadowMap,  vec2 uv, float zReceiver ) {
  ...
  int blockerCount = 0;
  float filterSize = ((zReceiver-0.1) / zReceiver) * float(W_LIGHT) * 10.0 / 2048.0;
  
  for(int i=0; i<BLOCKER_SEARCH_NUM_SAMPLES; i++){
    vec2 point = uv + poissonDisk[i] * filterSize;
    float shadowDepth = unpack(texture2D(shadowMap,point));
    if (shadowDepth < zReceiver){
      blockerValue += shadowDepth;
      blockerCount++;
    }
  }

  if(blockerCount == 0){
    return -1.0;//不是0 因为后面要用来除
  }
  if(blockerCount == NUM_SAMPLES){
    return 2.0;
  }
  return blockerValue/float(blockerCount);
}

float PCF(sampler2D shadowMap, vec4 coords) {
  float bias = Bias();
  float currentDepth = coords.z;
  float  filterSize=  1.0 / 2048.0 * 10.0; // * 1~10 边缘糊一点
  float visibility = 0.0;
  poissonDiskSamples(coords.xy);
  
  for(int i=0;i<NUM_SAMPLES;i++){
    vec2 shadowCoord = coords.xy+poissonDisk[i]*float(filterSize);
    float closestDepth = unpack(texture2D(shadowMap,shadowCoord));
...
float PCSS(sampler2D shadowMap, vec4 coords){
  ...
  if(avgBlockerDepth == 0.0) return 1.0;//没有遮挡直接返回1.0
  float filterSize = ((zReceiver-0.1) / zReceiver) * float(W_LIGHT) * 10.0 / 2048.0;
  
  // STEP 2: penumbra size //半影的大小
  ...
  for(int i=0;i<NUM_SAMPLES;i++){
    ...
    float closestDepth = unpack(texture2D(shadowMap,shadowCoord));
    visibility += closestDepth < zReceiver - bias ? 0.0 : 1.0;
  }
...
float useShadowMap(sampler2D shadowMap, vec4 shadowCoord){
  ...
  float bias = Bias();
  return depth < shadowCoord.z-bias ? 0.0 : 1.0;
}
...
void main(void) {
  ...
  // transform to [0,1] range 变换到[0,1]的范围 转为NDC空间[0,1]
